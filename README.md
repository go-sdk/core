...
v := 123
s := conv.PtrValue(&v)       // 123
z := conv.PtrValue[int](nil) // 0
```
...
`logx` 在包初始化时创建默认控制台日志。控制台默认写入标准输出；`APP__LOG__CONSOLE` 忽略大小写等于 `stderr` 时写入标准错误，其他值（包括空值）仍写入标准输出。应用读取配置后可以再次初始化，将日志同时写入滚动文件：
...
`log.no_color` 未配置时根据当前控制台输出是否为支持颜色的终端自动决定；`TERM=dumb` 或对应输出流被重定向时默认禁用颜色。显式配置始终优先。
